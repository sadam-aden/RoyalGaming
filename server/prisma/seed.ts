import { PrismaClient, StationType, ProductCategory, ProductType, StaffRole } from "@prisma/client";
...
async function main() {
  ...
  const products: ProductSeed[] = [
    ...packagesFor(ProductCategory.PLAYSTATION, StationType.PLAYSTATION, [
      { label: "30 Minutes", durationMin: 30, price: 25 },
      { label: "1 Hour", durationMin: 60, price: 45 },
      { label: "2 Hours", durationMin: 120, price: 80 },
    ]),
    ...packagesFor(ProductCategory.TABLE_GAMES, StationType.TABLE_GAME, [
      { label: "30 Minutes", durationMin: 30, price: 20 },
      { label: "1 Hour", durationMin: 60, price: 35 },
      { label: "2 Hours", durationMin: 120, price: 60 },
    ]),
    ...packagesFor(ProductCategory.SKATING, StationType.SKATING, [
      { label: "30 Minutes", durationMin: 30, price: 30 },
      { label: "1 Hour", durationMin: 60, price: 50 },
      { label: "2 Hours", durationMin: 120, price: 90 },
    ]),
    { name: "Espresso", category: ProductCategory.COFFEE, type: ProductType.ITEM, price: 15, stockQty: 100 },
    { name: "Cappuccino", category: ProductCategory.COFFEE, type: ProductType.ITEM, price: 20, stockQty: 100 },
    { name: "Latte", category: ProductCategory.COFFEE, type: ProductType.ITEM, price: 22, stockQty: 100 },
    { name: "Turkish Coffee", category: ProductCategory.COFFEE, type: ProductType.ITEM, price: 18, stockQty: 100 },
    { name: "Bottled Water", category: ProductCategory.CAFETERIA, type: ProductType.ITEM, price: 5, stockQty: 48 },
    { name: "Soft Drink (Can)", category: ProductCategory.CAFETERIA, type: ProductType.ITEM, price: 10, stockQty: 36 },
    { name: "Potato Chips", category: ProductCategory.CAFETERIA, type: ProductType.ITEM, price: 12, stockQty: 24 },
    { name: "Chocolate Bar", category: ProductCategory.CAFETERIA, type: ProductType.ITEM, price: 15, stockQty: 30 },
    { name: "Gaming Socks", category: ProductCategory.CAFETERIA, type: ProductType.ITEM, price: 25, stockQty: 0 },
    { name: "Energy Drink", category: ProductCategory.CAFETERIA, type: ProductType.ITEM, price: 18, stockQty: 20 },
  ];

  for (const p of products) {
    const existing = await prisma.product.findFirst({ where: { locationId: location.id, name: p.name, category: p.category } });
    if (!existing) {
