 ...
 * page length is measured from it — see below.
 */
export async function printReceipt(order: Order): Promise<void> {
  ...
  try {
    ...
    pageStyle.textContent = receiptPageRule(sheet);
...
/**
 * An `@page` rule sized to the rendered receipt.
 *
 * A receipt has no fixed length, so a static page height either wastes roll or
 * forces the receipt to be scaled down to fit — and scaling is exactly what
 * thins out the text on a thermal head. Measuring the sheet instead hands the
 * printer a page that already matches, at full size.
 *
 * Note that `size: <length> auto` is not valid CSS: mixing a length with the
 * `auto` keyword makes browsers drop the declaration and fall back to A4, which
 * is how receipts once ended up printing into the corner of a big sheet. Both
 * dimensions have to be real lengths, hence the measuring.
 */
function receiptPageRule(sheet: HTMLElement): string {
